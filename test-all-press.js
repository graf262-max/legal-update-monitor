...
async function testPipcPress() {
    ...
    try {
        ...
        for (const m of matches) {
            ...
            if (title && title.length >= 5 && isWithin7Days(dateStr) && !shouldExclude(title)) {
...
async function testMsitPress() {
    ...
    try {
        ...
        for (const m of matches) {
            ...
            if (title && !shouldExclude(title)) {
                const isMsit = /정보통신|인공지능|AI|데이터|전자금융|전자상거래|통신|소프트웨어|플랫폼|과학|기술/.test(title);
                if (isMsit) {
                    count++;
                    console.log(`  ${count}. ${title.substring(0, 50)}...`);
                }
...
async function testFscPress() {
    ...
    try {
        ...
        for (const m of matches) {
            ...
            if (title && isWithin7Days('') && !shouldExclude(title)) {
